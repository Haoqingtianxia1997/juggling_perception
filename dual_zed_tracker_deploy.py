...
class BallTrackingNode(Node):
    ...
    def __init__(self):
        ...
        # === 相机外参缓存（仅初始化一次，避免每帧重复读取） ===
        self.cam_pos_rel_cached, self.cam_rot_rel_cached = self.get_camera_extrinsics()
    ...
    def odometry_imu_callback(self, msg: Odometry):
        ...
        # 保存两套frame用于3D可视化
        self.latest_lidar_pose = (p_odom_world_compensated, R_diff @ R_lidar_rot)
        self.latest_pose = (self.latest_base_pos.copy(), R_odom_world)
        self.latest_body_pose = self.latest_pose
        self.pose_positions.append(self.latest_base_pos.copy())
    ...
    def update_visualization(self):
        ...
        if self.enable_3D_visualization and hasattr(self, 'frame_ax'):
            ...
            pts = np.asarray(self.pose_positions, dtype=np.float64)
            if pts.shape[0] > 0:
                ax.plot(pts[:, 0], pts[:, 1], pts[:, 2], color='tab:blue', linewidth=1.8, label='trajectory')

            if self.latest_pose is not None:
                pos, _ = self.latest_pose
                ax.scatter([pos[0]], [pos[1]], [pos[2]], color='tab:red', s=90, edgecolors='k', linewidths=0.8, label='current_pose')

            if self.latest_lidar_pose is not None:
                lidar_pos, _ = self.latest_lidar_pose
                ax.scatter([lidar_pos[0]], [lidar_pos[1]], [lidar_pos[2]], color='tab:orange', s=70, edgecolors='k', linewidths=0.6, label='lidar_pose')

            self._draw_world_frame(ax, origin=np.zeros(3), scale=0.25)

            if self.latest_pose is not None:
                pos, rot = self.latest_pose
                self._draw_pose_frame(ax, pos, rot, scale=0.15)
            ...
            self._set_equal_3d_axes(ax, pts_with_origin, margin=0.3)

            ax.text2D(
                0.02, 0.98,
                f"samples={pts.shape[0]}\n"
                f"status={'receiving' if self.latest_pose is not None else 'waiting'}",
    ...
    @staticmethod
    def _draw_world_frame(ax, origin=np.zeros(3), scale: float = 0.25):
        ox, oy, oz = origin
    ...
    @staticmethod
    def _set_equal_3d_axes(ax, pts: np.ndarray, margin: float = 0.3):
        if pts.shape[0] == 0:
            ax.set_xlim(-0.5, 0.5)
            ax.set_ylim(-0.5, 0.5)
            ax.set_zlim(-0.5, 0.5)
