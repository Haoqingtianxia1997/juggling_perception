...
class TrajectoryVisualizer:
    ...
    def __init__(self, trajectory_json_path):
        ...
        runtime_cfg = camera_config.get('tracker', {}).get('runtime', {})
        self.num_balls = int(runtime_cfg.get('num_balls', 1))
        
        print(f"Visualize ICP: {self.visualize_icp}")
        print(f"Configured num_balls from yaml: {self.num_balls}")
        ...
        self.gt_max_x = 0.7
        # 多球场景下，GT从点云中按“球簇 + 距离先验”匹配
        self.gt_expected_balls = max(1, int(self.num_balls))
        self.gt_match_max_distance = 0.18
        self.gt_dbscan_eps = 0.03
        self.gt_dbscan_min_points = 8
    ...
    def _select_gt_cluster_points(self, pcd_for_gt, prior=None):
        """
        在GT候选点云中做DBSCAN聚类，针对多球场景按先验位置匹配目标球。

        Args:
            pcd_for_gt: 已完成基础过滤/离群点剔除的点云
            prior: 先验位置（通常为detection_pos）

        Returns:
            (selected_points, info)
            - selected_points: 选中的簇点云 Nx3，若无可用结果则为None
            - info: {'candidate_clusters': int, 'selected_distance': float|None, 'reason': str|None}
        """
        points = np.asarray(pcd_for_gt.points)
        if points.shape[0] < max(8, self.gt_dbscan_min_points):
            return points, {
                'candidate_clusters': 1 if points.shape[0] > 0 else 0,
                'selected_distance': None,
                'reason': 'too_few_points_for_clustering'
            }

        try:
            labels = np.asarray(
                pcd_for_gt.cluster_dbscan(
                    eps=float(self.gt_dbscan_eps),
                    min_points=int(self.gt_dbscan_min_points),
        ...
        for lab in np.unique(labels[valid]):
            ...
            if idx.size < int(self.gt_dbscan_min_points):
                continue
            pts = points[idx]
            clusters.append({
                'indices': idx,
        ...
        max_candidates = int(max(1, self.gt_expected_balls))
        ...
        prior_arr = np.asarray(prior, dtype=np.float64).reshape(3)
        best = None
        best_dist = None
        for c in candidate_clusters:
            dist = float(np.linalg.norm(c['center'] - prior_arr))
        ...
        if best is None:
            return None, {
                'candidate_clusters': len(candidate_clusters),
                'selected_distance': None,
                'reason': 'no_best_cluster'
            }

        if best_dist is not None and best_dist > float(self.gt_match_max_distance):
            return None, {
                'candidate_clusters': len(candidate_clusters),
                'selected_distance': float(best_dist),
                'reason': 'match_distance_exceeded'
            }

        return points[best['indices']], {
            'candidate_clusters': len(candidate_clusters),
            'selected_distance': float(best_dist) if best_dist is not None else None,
            'reason': None
        }
    ...
    def estimate_ground_truth_from_point_cloud(self, pcd, frame=None):
        ...
        if frame is not None:
            ...
            prior = np.asarray(frame['detection_pos'], dtype=np.float64)
        ...
        # 多球场景：先做聚类并按先验位置匹配目标球簇
        selected_points, cluster_info = self._select_gt_cluster_points(pcd_for_gt, prior=prior)
        if selected_points is None or selected_points.shape[0] < 8:
            return {
                'position': None,
                'rmse': None,
                'inlier_points': 0,
                'cluster_points': int(0 if selected_points is None else selected_points.shape[0]),
                'total_points': int(np.asarray(pcd_for_gt.points).shape[0]),
                'candidate_clusters': int(cluster_info.get('candidate_clusters', 0)) if cluster_info is not None else 0,
                'cluster_match_distance': None if cluster_info is None else cluster_info.get('selected_distance'),
                'cluster_match_reason': None if cluster_info is None else cluster_info.get('reason')
            }

        prefiltered_points = np.asarray(selected_points, dtype=np.float64)
...
使用示例:
  ...
  python3 visualize_trajectory_3d.py trajectory_data/trajectory_tracker0_20260309_123456_0000.json --start 10 --end 20
    ...
    parser.add_argument('--gt-balls', type=int, default=None,
                       help='GT聚类候选球数（默认读取Tracker_config.yaml中的tracker.runtime.num_balls）')
    parser.add_argument('--gt-match-max-dist', type=float, default=0.18,
                       help='GT簇匹配先验最大距离（米，默认0.18）')
    parser.add_argument('--gt-dbscan-eps', type=float, default=0.03,
                       help='GT聚类DBSCAN eps（米，默认0.03）')
    parser.add_argument('--gt-dbscan-min-points', type=int, default=8,
                       help='GT聚类DBSCAN min_points（默认8）')
    
    args = parser.parse_args()

    def configure_visualizer(viz):
        """应用通用可视化/GT配置。"""
        viz.enable_x_filter = args.enable_x_filter
        viz.x_filter_threshold = args.x_max
        viz.show_ground_truth_marker = not args.hide_gt_marker
        if args.gt_balls is not None:
            viz.gt_expected_balls = max(1, int(args.gt_balls))
        viz.gt_match_max_distance = float(args.gt_match_max_dist)
        viz.gt_dbscan_eps = float(args.gt_dbscan_eps)
        viz.gt_dbscan_min_points = max(3, int(args.gt_dbscan_min_points))

        if args.cam_height != 0.3 or args.cam_forward != 0.0 or args.cam_lateral != 0.0:
            camera_rotation_in_body = viz.camera_to_body_transform[:3, :3]
